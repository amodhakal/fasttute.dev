...
export function getTranscriptValuesFromSegment(
  segment: AnySegment
): TranscriptValues {
  if (
...
export function extractTextFromSegment(segment: AnySegment): string {
  if (
    segment &&
    "transcript_segment_renderer" in segment &&
    segment.transcript_segment_renderer
...
export type TranscriptSegmentRenderer = {
  snippet?: Snippet;
  text?: string;
  runs?: TextRun[];
  start_ms?: string;
